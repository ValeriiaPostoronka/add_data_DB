...
const getIrradianceStatsForDay = async (day, table, column) => {
  const query = `
    SELECT 
        "${column}"
    FROM 
  ...
  return result.map(row => parseFloat(row[column]));
}
...
const calculateTest = async (d, a, t, c) => {
  ...
  const table = t;
  const column = c;
  const stats = await getIrradianceStatsForDay(day, table, column);

  const shapiroWilkResult = shapiroWilkTest(stats);
  console.log(`W: ${shapiroWilkResult.w}`);
  console.log(`p: ${shapiroWilkResult.p}`);

  const isNormal = comparePandAlpha(alpha, shapiroWilkResult.p);
  let message;
  if (isNormal) {
    message = `Дані за ${day} день нормально розподілені.`;
    console.log(message);
  }
  else {
    message = `Нормальність даних з очікуванням похибки в ${alpha} не приймається за ${day} день.`;
    console.log(message);
  }

  return { W:shapiroWilkResult.w, p:shapiroWilkResult.p, message, isNormal };
};
